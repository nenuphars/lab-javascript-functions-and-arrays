  ...
  if (mixedArr.length === 0) {
    ...
    mixedArr.forEach(function (element) {
      if (typeof element === "object") {
        throw new Error(
          "Array elements can only be of type number, string or boolean"
        );
      } else {
...
function averageWordLength(wordsArr) {
  ...
  if (wordsArr.length === 0) {
    ...
    wordsArr.forEach(function (element) {
      wordsLengths += element.length;
    });
    return wordsLengths / wordsArr.length;
  }
}
...
function uniquifyArray(wordsUnique) {
  ...
  if (wordsUnique.length === 0) {
    ...
    wordsUnique.forEach(function (element) {
      if (uniquifiedArray.includes(element) === false) {
...
function doesWordExist(wordsFind, searchTerm) {
  if (wordsFind.length === 0) {
    ...
    if (wordsFind.includes(searchTerm)) {
      return true;
    } else {
      return false;
    }
